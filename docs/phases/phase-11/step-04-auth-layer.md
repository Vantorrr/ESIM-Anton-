...
- `admin/lib/auth.ts` — token storage helpers + `auth:logout` event contract
- `admin/components/AuthProvider.tsx` — auth state machine и context actions
- `admin/components/AuthGuard.tsx` — protected-route spinner/redirect contract
- `admin/lib/api.ts` — interceptors переведены на `getToken()` / `clearToken()` / logout event dispatch

## Тестирование / Верификация

- `npm run build` в `admin` → проходит
- `npm run lint` в `admin` → проходит без warnings/errors
- `api.ts`: request interceptor читает Bearer token через `getToken()`
- `api.ts`: 401 response вызывает `clearToken()` + dispatch `'auth:logout'` event, без `window.location.reload()`
- `AuthProvider` / `AuthGuard` компилируются как изолированные client-модули; интеграция в layouts остаётся на Step 05
- Полный auth smoke (`login`, `logout`, `401 redirect`, `returnUrl`) — в Step 05 после wiring в route structure
