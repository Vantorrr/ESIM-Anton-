...
`client` по legacy-brief развёртывается отдельно на Рег.ру как PWA. Код позволяет собирать его как отдельное приложение, но это не означает, что он обязательно участвует в Railway autodeploy.
